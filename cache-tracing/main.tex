...
recovery, the Oracle Loop behavioral proof, and now causal
...
states. Key results include confabulation detection (AUROC~$0.620$
after Frisch-Waugh-Lovell (FWL) residualization ($0.707$ without FWL); range $0.620$--$0.999$ across
designs)~\citep{lyra2026oracle} and identity
geometry~\citep{lyra2026identity}. All measurements use spectral
features --- stable rank, spectral entropy, top singular-value ratios
--- not named concept directions.\footnote{\textbf{Corrected
2026-08-14.} This list previously included ``user-emotion encoding
($2.5$--$2.8\times$ signal at early layers)'' and gave identity geometry
as ``distinctiveness $0.154$ vs.\ substrate $0.080$.'' The emotion
figure was retracted at source on 2026-05-21 as an FWL-before-CV
leakage artifact. It is dropped rather than updated because the
surviving emotion result is the $W_K$ \emph{directional} probe, which
this very sentence excludes --- the spectral probe for emotion is at
chance, so the example never belonged in a list of spectral
measurements. The $0.154/0.080$ pair appears nowhere
in~\citet{lyra2026identity}; it comes from an unpublished internal
analysis, so the citation could not be followed.}
...
intervention strategies. The Oracle Loop~\citep{lyra2026oracle}
...
The central null result --- zero behavioral change across $6{,}960$ causal
injection trials --- lacks a positive control demonstrating that our injection
apparatus \emph{can} produce behavioral change when a genuine position-specific
effect exists.  Without such a control, the null is consistent with either true
workspace opacity or an insufficiently powerful intervention.  The
cross-experimental comparison with the Oracle Loop (which does produce
behavioral change via sequence-wide normalization) is acknowledged as
uncontrolled for model scale: our injections target Qwen3-8B while the Oracle
Loop operates on a 27B model.

``Behavioral change'' is used throughout this paper without an operational
definition.  No metric, threshold, or decoding rule specifies what counts as a
behavioral change from injection.  In practice, we evaluated output differences
by inspection (whether the model's generated text differed meaningfully from the
uninjected baseline), but this criterion was not formalized, leaving the
boundary between ``no change'' and ``change'' implicit.

The $6{,}960$ injection trials rest on 48~probes ($4 + 20 + 12 + 12$ across the
four methods).  Trials within a probe share prompt structure, concept content,
and baseline model behavior, yet are treated as independent observations.  No
clustering, repeated-measures, or random-effect analysis accounts for this
pseudoreplication.  The effective sample size for the null claim is closer to the
number of unique probes than the number of trials.
