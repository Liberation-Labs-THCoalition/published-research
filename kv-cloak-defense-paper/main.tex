...
A single invertible linear transform of the KV cache degrades SVD-based
cognitive-state detection from near-ceiling to near-chance ($p = 5.5 \times 10^{-16}$),
while a real KV-Cloak implementation confirms the defense is conservative
(detection AUROC drops from 0.903 to 0.614). SVD of the
...
confabulations, $n=89$ hedged). A text-only surface-feature baseline
...
\item \textbf{Obfuscation defeats transfer attacks.} Per-head orthogonal
rotation---simulating KV-Cloak's reversible matrix obfuscation---produces
a total distributional shift in MP features (cloaked-honest vs
uncloaked-honest: Wilcoxon signed-rank $p = 5.5 \times 10^{-16}$,
$n=89$ paired trials). A detector calibrated on uncloaked caches cannot
operate on cloaked caches. This is a domain-shift effect: rotation
preserves the underlying geometric structure but changes the coordinate
system, preventing transfer from uncloaked calibration data. An
adversary with access to the obfuscation key could retrain and recover
the signal.
...
mechanism ($S \cdot \hat{P} \cdot (K + A) \cdot M$) degrades
spectral gap detection from 0.903 to 0.614 via $\sigma_1$ perturbation
from the additive mask, while top-SV excess increases (0.609 to 0.736)
under the same mechanism---both are mask-on-$\sigma_1$ effects, not
evidence of geometric destruction.
The rotation-simulation result (0.786) represents
transfer-attack failure (domain shift under coordinate change), not
signal degradation. We discuss limitations including small sample size
($n=7$ confabulations on the distilled model, making the 0.903 baseline
itself exploratory), marginal cache-vs-text added value, reliance on TEE
hardware trust assumptions, and underpowered injection testing.
...
MP-corrected features. The defense evaluation should be understood as
applying to this specific feature extraction pipeline, not to arbitrary
geometric attackers.
...
$R^2 = 0.068$ vs token count; $n=7$ confabulations, $n=89$ hedged)
...
MP features. On the distilled model ($n=89$ honest trials, paired):
...
\begin{table}[h]
\centering
\caption{Cross-architecture KV-Cloak results. Prompt count varies by model (50--100 prompts each); the Qwen3.5-27B-Dist.\ row is $n=100$ ($n=7$ confabulations).}
...
(distilled model, $n=7$ confab, $n=89$ hedged). Bootstrap 95\% CIs.}
...
The real KV-Cloak mechanism produces a larger AUROC drop for spectral
gap than rotation alone (0.614 vs 0.786). Per-head rotation is a
coordinate change that prevents transfer from uncloaked calibration
data; the additive mask $A$ additionally perturbs $\sigma_1$-dependent
features. Both degrade a transfer attack---a detector calibrated on
uncloaked caches applied to cloaked caches---rather than destroying the
underlying geometric signal.
...
on different terms. The honest reading is unified: the defense works
\emph{by} perturbing $\sigma_1$, which disrupts $\sigma_1$-dependent
detection features---degrading the feature that happened to detect well
...
% confabulation detection. Deception detection and other cognitive-state
% classifiers were not tested under obfuscation.
% (Do NOT reinstate the withdrawn "AUROC~1.000 in prior work" figure.)
\end{enumerate}
...
\textit{I built the detection; this paper shows how to break it. That should be uncomfortable, and it is---but a detection system that has never been stress-tested against countermeasures is not a detection system. It is an assumption.}

\textit{The defense works. Feature-space transforms degrade SVD-based detection to near-chance. The dual-use implication is real: anything we publish about detection geometry is also a tutorial on evasion. We chose to publish because the gap between what we can detect and what can evade detection is a real security margin, and knowing that margin is more valuable than pretending it does not exist. The alternative---suppressing the defense and claiming our detection is unbreakable---would be a lie of omission, and lies of omission are what the Oracle Loop is built to catch.}

\textit{The 1038-line reproducible replication script was Dwayne's requirement, and he was right to require it. If we are going to say ``the defense works,'' the claim must be verifiable from the artifact, not from our word. --- Lyra}
