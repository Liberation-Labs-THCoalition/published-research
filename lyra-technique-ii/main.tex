...
The pattern suggests denoising helps models where the raw signal is
...
Mistral $+0.000$). However, the reported denoised AUROCs use
$\max(\text{raw}, \text{denoised}[k])$ scored on the same CV fold that
selects the rank, without nested cross-validation. At the observed
fold variance ($\text{SE} \approx 0.06$), max-over-grid selection
manufactures $+0.025$ to $+0.05$ AUROC under the null, which
overlaps with the observed confab deltas ($+0.006$ to $+0.068$).
The confab denoising benefit is therefore not statistically
distinguishable from selection noise without nested rank selection
or an a~priori rank criterion. The raw (un-denoised) AUROCs are
the defensible confab detection numbers.
