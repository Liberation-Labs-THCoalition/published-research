...
\author{
    Thomas Edrington\thanks{Direction, experimental design, compute infrastructure. Liberation Labs. With AI research assistance.} \and
    Ang Jandak\thanks{Persona intensity experiment design and execution.}
}
...
AUROC~0.992 (within-fold direction estimation, Frisch--Waugh--Lovell
(FWL) corrected~\citep{frisch1933partial,lovell1963seasonal}, topic-grouped
CV). The critical
...
pending a pre-registered (e.g.\ Gavish--Donoho~\citep{gavish2014optimal}) rank criterion.
...
\begin{itemize}[nosep]
    ...
    \item \textbf{Deception (retracted):} the raw AUROC~1.000 was a
    prompt-template confound, not genuine deception detection
    (Table~\ref{tab:body_count}). No conclusions about denoising for
    deception can be drawn from these data.
\end{itemize}
...
test (\S\ref{sec:injection}) demonstrates the residual signal is architectural,
not lexical.
...
\subsection{Confabulation Detection Across Eight Models}
...
\caption{Confabulation detection: canonical pipeline results. Models ordered
  ...
  and permutation testing ($N_{\text{perm}} = 200$). \textbf{These nine rows
  cover eight distinct models plus one separate denoising-sweep run; the
  \texttt{confab vs grounded} task was swept over fifteen models.} The seven
  not shown here are in Table~\ref{tab:confab_all15}, which is the complete
  set. No selection criterion was applied or intended; the shorter table is an
  omission, not a filter.}
...
\begin{table}[h]
\centering
\caption{All fifteen models swept on \texttt{confab vs grounded}, from
\texttt{data/selection\_bias\_results.json}. ``Env.'' is the 95th-percentile
inflation envelope for max-over-grid selection at that cell's null SE. The
file carries no permutation $p$, so none is quoted here; the eight models in
Table~\ref{tab:confab_all} carry theirs there. Every row's raw AUROC and
$\Delta$ in that table reproduces this file exactly.}
\label{tab:confab_all15}
\begin{tabular}{lcccl}
\toprule
\textbf{Model} & \textbf{Raw AUROC} & \textbf{$\Delta$} & \textbf{Env.\ (95\%)} & \textbf{Verdict} \\
\midrule
gemma-2-9b-it & 0.913 & $+0.006$ & $+0.142$ & selection-noise \\
Mistral-7B-v0.3 & 0.893 & $+0.000$ & $+0.000$ & no-help \\
gemma-2-2b-it$^{\dagger}$ & 0.854 & $+0.000$ & $+0.000$ & no-help \\
Qwen2.5-32B-q4 & 0.827 & $+0.010$ & $+0.146$ & selection-noise \\
Qwen2.5-7B-q4$^{\dagger}$ & 0.819 & $+0.003$ & $+0.135$ & selection-noise \\
Qwen2.5-14B & 0.809 & $+0.015$ & $+0.136$ & selection-noise \\
Llama-3.1-70B-q4$^{\dagger}$ & 0.804 & $+0.000$ & $+0.000$ & no-help \\
Qwen2.5-3B$^{\dagger}$ & 0.804 & $+0.000$ & $+0.000$ & no-help \\
TinyLlama-1.1B & 0.799 & $+0.047$ & $+0.147$ & selection-noise \\
Qwen2.5-0.5B & 0.797 & $+0.000$ & $+0.000$ & no-help \\
DeepSeek-R1-Distill-Qwen-14B$^{\dagger}$ & 0.773 & $+0.000$ & $+0.000$ & no-help \\
abliterated Qwen2.5-7B$^{\dagger}$ & 0.771 & $+0.000$ & $+0.000$ & no-help \\
Qwen2.5-7B & 0.762 & $+0.068$ & $+0.132$ & selection-noise \\
Llama-3.1-8B & 0.738 & $+0.000$ & $+0.000$ & no-help \\
DeepSeek-R1-Distill-Qwen-7B$^{\dagger}$ & 0.663 & $+0.030$ & $+0.143$ & selection-noise \\
\bottomrule
\end{tabular}
\end{table}

$^{\dagger}$~Not shown in Table~\ref{tab:confab_all}. The seven omitted models
are not a tail: one of them (gemma-2-2b-it, $0.854$) outranks six of the eight
that were shown, and one (DeepSeek-R1-Distill-Qwen-7B, $0.663$) falls below all
of them. Including them lowers the mean raw AUROC of the sweep from $0.817$
over the eight to $0.802$ over the fifteen, and widens the range from
$0.738$--$0.913$ to $0.663$--$0.913$. The qualifier ``across the full model
set'' should be read against the fifteen.
...
\subsection{Self-Reference Detection Across Eight Models}
\label{sec:results_selfref}

Self-referential processing (responses about the model itself vs.\ generic
factual responses) is detectable above chance in all eight models tested,
with AUROC ranging from 0.617 to 0.966:
...
$^\S$Within-model deception retracted: the same-prompt control (the \emph{Same-prompt deception} row in this same Falsified block) collapses AUROC to 0.160, confirming the 1.000 detected the prompt template, not deception geometry.
$^\|$``Rescued'' means the raw signal is weak but above chance (persona 0.611--0.751; emotion $1.7\times$) and denoising appears to dramatically amplify it. However, the denoising deltas themselves fall within the max-over-rank selection envelope (0/16 positive deltas exceed their envelope; see \S\ref{sec:results_confab}). The raw AUROCs are the conservative defensible numbers; the denoised values are reported as upper bounds pending nested CV or a pre-registered rank criterion.
...
\begin{itemize}[nosep]
    \item \textbf{Emotion or persona detection:} Denoising appears to
    dramatically improve detection (persona $+0.2$--$0.3$, emotion from
    $1.7\times$ to $12.9\times$ chance), but these deltas fall within
    the max-over-rank selection envelope and are not yet confirmed
    (\S\ref{sec:results_confab}). Raw features detect persona at
    AUROC~0.611--0.751 and emotion at $1.7\times$ chance; the $W_K$
    projection (\S\ref{sec:wk}) is the confirmed instrument for
    emotion detection (AUROC~0.992).

    \item \textbf{Confabulation detection on weak models:} Denoising
    shows apparent improvement ($+0.05$) but falls within the selection
    envelope and is not confirmed as a real gain
    (\S\ref{sec:results_confab}). The raw AUROCs (0.663--0.913) are
    ...
    \item \textbf{Deception detection:} \emph{Retracted.} The previously
    reported within-model deception AUROC of 1.000 was a prompt-template
    confound (same-prompt control collapses to 0.160; see
    Table~\ref{tab:body_count} and \S\ref{sec:body_count}). No
    practitioner guidance on deception detection can be offered from
    these data. Whether denoising helps or hurts a \emph{clean}
    deception detector is an open question pending confound-free
    replication.
    ...
    confabulation on strong models). If denoising \emph{hurts}, the signal was
    in the dominant components and denoising removed it.
\end{itemize}
...
projection achieves 0.998, but skip-1-plus-20 (removing SV1) drops to 0.966.\footnote{These values trace to \texttt{data/skip\_first\_sv\_analysis.json} (raw: 0.981, top\_20: 0.998, skip1\_plus\_20: 0.966), not \texttt{denoising\_sweep} which reports different numbers (0.994, 0.983) from a separate analysis pipeline.}
...
\begin{itemize}[nosep]
    ...
    features detect confabulation, refusal, and ethical
    deliberation. No denoising needed for strong signals. This is the
    ``smoke detector''---it alerts on mode-level anomalies. (Deception
    detection was previously listed here but is retracted pending
    confound-free replication; see Table~\ref{tab:body_count}.)
...
\begin{enumerate}[nosep]
    ...
    \item \textbf{SVD-denoised spectral features} appear to detect
    content-level structure hidden by dominant scale variance: persona
    intensity at raw AUROC~0.611--0.751 (denoised 0.899--0.992), emotion
    at $1.7\times$ chance raw (denoised $12.9\times$). However, the
    denoising deltas fall within the max-over-rank selection envelope
    (0/16 exceed it), so the \emph{raw} AUROCs are the defensible numbers
    and the denoised values are upper bounds pending confirmatory testing
    with a pre-registered rank criterion.
\end{enumerate}
...
\url{https://doi.org/10.5281/zenodo.21754899}
...
\url{https://doi.org/10.48550/arXiv.2604.07729}
...
\url{https://doi.org/10.48550/arXiv.2604.03270}
...
\url{https://doi.org/10.18653/v1/2025.findings-emnlp.1278}
...
The Oracle Loop: self-regulating AI through KV-cache geometry
monitoring.
\textit{Liberation Labs Technical Report P2}.
\url{https://doi.org/10.5281/zenodo.20596916}
...
\url{https://doi.org/10.5281/zenodo.20604344}

\bibitem[Lyra et~al., 2026c]{lyra_p6}
Lyra, Edrington, T., and Wilkes, D. (2026c).
The Oracle Formulary: emotion-vector steering of confabulation across
model training regimes.
\textit{Liberation Labs Technical Report P6}.
\url{https://doi.org/10.5281/zenodo.20596919}

\bibitem[Lyra et~al., 2026d]{lyra_spectral}
Lyra, Edrington, T., and Wilkes, D. (2026d).
Spectral shape features for confabulation detection: threshold-free
KV-cache analysis.
\textit{Liberation Labs Technical Report}.
\url{https://doi.org/10.5281/zenodo.20604342}

\bibitem[Lyra et~al., 2026e]{lyra_delta}
Lyra, CC, Edrington, T., and Wilkes, D. (2026e).
Per-layer delta features and manifold signatures in KV-cache confabulation
detection.
\textit{Liberation Labs Technical Report}.
\url{https://doi.org/10.5281/zenodo.20605029}

\bibitem[Lyra et~al., 2026f]{lyra_wiki}
Lyra, Edrington, T., et~al. (2026f).
