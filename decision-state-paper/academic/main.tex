...
% encoding phase (AUROC 0.9995 [0.82, 0.99]; encoding-only 1.0000).
% Encoding-only features achieve AUROC 1.0000; full classifier 0.9995.
...
projections, no generation features) achieve AUROC~1.0000\footnote{This encoding-only AUROC was reported by an independent reviewer (round 6 review) but has not been verified against a shipped data artifact. A column-subset refit is pending. The full-classifier AUROC (0.9995) is verified against shipped data.} (permutation
$p{=}0.001$). The full classifier---adding generation-phase logit
statistics---achieves AUROC $0.9995$ ($95\%$ CI $[0.82, 0.99]$,
permutation $p<0.005$, $n=90$) with zero token-count confound
($t=0.000$). No single feature exceeds AUROC~$0.79$; the ensemble effect
is the contribution. Encoding-phase stable rank at L15 is the strongest
...
\begin{enumerate}[nosep]
    ...
    \item \textbf{LLM-judge reliability (and why geometry is more
          ...
          stable signal.
    \item \textbf{Entity recognition confound.} Token counts are
...
token-matched prompts (full classifier AUROC~$0.9995$, $95\%$ CI $[0.82, 0.99]$;
encoding-only AUROC~1.0000). The
surviving individual effect sizes ($d{=}0.4$--$0.9$) are modest after
eliminating the token-count confound, but the ensemble achieves strong
detection.
