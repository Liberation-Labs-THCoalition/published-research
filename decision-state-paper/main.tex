...
% encoding phase (AUROC 0.93 [0.82, 0.98] on token-matched prompts).
% The model's epistemic state is readable before generation begins.
...
at AUROC 0.93 [0.82, 0.98] before a single token is generated---the model's
epistemic state is readable from its cache geometry. Using token-matched prompt
...
logit features achieves AUROC $0.93$ ($95\%$ CI $[0.82, 0.98]$,
...
\begin{enumerate}[nosep]
    ...
    \item \textbf{LLM-judge reliability (and why geometry is more
          ...
          is the bottleneck \citep{zheng2023judging}. The capable judge
...
token-matched prompts (AUROC~$0.93$, $95\%$ CI $[0.82, 0.98]$). The
