...
% unknown entities (AUROC 0.9995 [0.82, 0.99]; encoding-only 1.0000) but entity recognition
...
AUROC 1.0000\footnote{This encoding-only AUROC was reported by an independent reviewer (round 6 review) but has not been verified against a shipped data artifact. A column-subset refit is pending. The full-classifier AUROC (0.9995) is verified against shipped data.} (permutation $p{=}0.001$), and the full classifier---adding
generation-phase logit statistics---achieves AUROC 0.9995
($95\%$ CI $[0.82, 0.99]$, permutation $p<0.005$, $n=90$,
...
where encoding-only features achieve perfect separation.
...
\begin{center}
\textbf{AUROC = 0.9995} \quad 95\% CI $[0.82, 0.99]$ \quad
Permutation $p < 0.005$ (0/200 shuffles)
\end{center}

FWL residualization on prompt length is applied; the post-FWL AUROC is 0.9995. A pre-FWL AUROC was not computed (0.977 in earlier drafts was combined accuracy, not AUROC),
...
encoding phase (encoding-only AUROC~1.0000; full classifier AUROC~$0.9995$, $95\%$ CI $[0.82, 0.99]$), acknowledges
...
unknown entities from encoding-phase features alone (AUROC~1.0000), using
...
\begin{enumerate}[nosep]
    ...
    \item \textbf{Entity recognition confound (major).} Token counts are
          ...
          features) achieves AUROC~1.0000---\emph{higher} than the full
...
projections, and logit features achieve AUROC~$0.9995$ ($95\%$ CI
$[0.82, 0.99]$) at separating known from unknown entities on
token-matched prompts; encoding-only features (no generation-phase logit
statistics) achieve AUROC~1.0000. An entity deconfounding experiment (same real
