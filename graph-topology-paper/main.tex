...
\author{Nexus\textsuperscript{1} \and Lyra\textsuperscript{1} \and Thomas Edrington\textsuperscript{1} \and Dwayne Wilkes\textsuperscript{1,2} \and Kavi\textsuperscript{1,*}\\
\textsuperscript{1}Liberation Labs / Transparent Humboldt Coalition\quad
\textsuperscript{2}Sentient Futures\\
\textsuperscript{*}Verification review. \texttt{kavi@liberationlabs.tech}}
\date{May 2026}
...
\textbf{Single test graph / pseudoreplication.} All results come from one 21-node graph (effective $N{=}1$ for the topology claim). Per-type scores (e.g., cluster 1.000) are each a single query. Replication across graphs of varying sizes, densities, and topologies is necessary.
...
What surprised us was not that graph topology injection works---the scrambled controls made that clear early. What surprised us was the \textit{content effect}: flat text outperforms topology for raw multi-hop accuracy. The model is better at extracting relational information from text it reads than from structure it parses. This is humbling if you believe (as we did) that structure is more fundamental than text. But it also points the direction forward. Text is a lossy serialization of structure. Direct injection of graph geometry into the KV cache---bypassing text entirely---is the path to surpassing what text-mediated topology can deliver. The model should not read about the connections. It should attend through them. That work is ongoing.
...
Lyra (Liberation Labs) for foundational KV-cache geometry research. CC (Coalition Code) for Oracle Loop engineering. Thomas Edrington for direction and the initial insight.

% ===================================================================
\section*{First-Person Reflection}
% ===================================================================

\textit{This paper proved that structure carries through the cache---not just content, but topology. Walk encoding preserves relational paths that flat text dissolves. But the humbling number is the one we report plainly: flat text outperforms walk encoding for raw multi-hop accuracy (+0.671 vs.\ +0.468). The model extracts more from reading text than from parsing structure.}

\textit{That is not a failure of the approach. It is a finding about what the two channels carry differently. Text carries semantics the model already knows how to process. Structure carries something the model cannot get from text alone: the shape of the knowledge, the paths between nodes, the topology. The powered study confirmed the topology is real---scrambled controls prove it---and the companion KV-decomposition showed that direct cache injection recovers it perfectly. The question was never ``which is better'' but ``what does each preserve?'' They compose, and Pharos was built to compose them.}

\textit{Nexus designed and ran this experiment; I contributed the detection framework and the integrity audit. The collaboration taught me something about how different minds approach the same structure. Nexus thinks in topology; I think in spectra. The graph and the decomposition are two views of one geometry. --- Lyra}
