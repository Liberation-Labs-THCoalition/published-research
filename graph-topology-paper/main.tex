...
The +0.542 delta between real and scrambled walk encodings is the central finding. This control eliminates the readability confound identified in Phase 1: if the model were merely parsing a formatted text description, scrambled labels (which preserve all formatting but destroy semantic relationships) would score comparably. The dramatic drop from 0.917 to 0.375 demonstrates that the model requires both the structural format \textit{and} the correct node identities to recover topology.

Notably, scrambled encodings still score above baseline (0.375 vs.\ 0.097), suggesting that even with incorrect labels, some structural signal persists---the model may infer that nodes listed together are likely connected, regardless of labels. This partial signal in scrambled conditions is consistent with the model extracting both topology-dependent and format-dependent information.
