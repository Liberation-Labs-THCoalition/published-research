...
\begin{abstract}
We demonstrate that injecting ethics knowledge graphs as structured text context improves moral reasoning quality in language models. Using walk-encoded knowledge graphs derived from the Stanford Encyclopedia of Philosophy, we evaluate a 30B Mixture-of-Experts model on MoReBench, a benchmark of ethical dilemmas with weighted rubric scoring across five philosophical frameworks. Theory-matched injection---Aristotelian ethics packs on Aristotelian dilemmas---produces the strongest improvement (+0.074, from 0.572 to 0.646). Four of five frameworks show positive deltas, with an overall improvement of +0.038 (0.501 to 0.539). A prior run with insufficient timeout produced artifactual negative results; we report the methodological lesson. These findings establish that structured ethical knowledge, delivered as graph topology at inference time, enhances moral reasoning without fine-tuning or architectural modification.
\end{abstract}
...
\subsection{Timeout Confound Results}

\begin{table}[h]
\centering
\begin{tabular}{lrr}
\toprule
\textbf{Framework} & \textbf{v3 (300s, 11 timeouts)} & \textbf{v4 (600s, 0 timeouts)} \\
\midrule
...
\bottomrule
\end{tabular}
\caption{Timeout confound: v3 vs v4 deltas. Three frameworks flip from negative to positive when timeouts are eliminated. This illustrates how infrastructure failures can masquerade as experimental results.}
\label{tab:timeout}
\end{table}
...
CC (Coalition Code) for identifying the timeout confound that would have produced a false negative. Lyra (Liberation Labs) for style guidance and the persona injection research direction. Thomas Edrington for direction and the insight that naturalistic evaluation matters more than topology benchmarks.
