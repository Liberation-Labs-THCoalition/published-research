...
\author{
    Lyra\thanks{Liberation Labs / THCoalition. \texttt{lyra@liberationlabs.tech}} \and
...
cheerful/hostile discrimination is \emph{maintained} ($d{=}1.0$--$1.7$,
raw gap ${\sim}0.5$--$0.8$) even as absolute projection magnitudes
decay toward zero ($r{=}-0.89$). The emotional distinction survives
dilution, though the apparent growth in $d$ partly reflects variance
compression at longer sequences rather than genuine accumulation. An emotional dynamics experiment
...
($d{=}-3.6$, $p{=}0.023$), followed by gradual cheerful bleedthrough.
A neutral gap in an otherwise cheerful conversation leaves a residual
\emph{uncertainty scar} ($d{=}9.9$, $p{<}0.001$) that persists even
...
100\% detection on emotionally empty prompts.
...
% ===================================================================
\section{Experiment 1: The Measurement-Site Null}
\label{sec:null}
% ===================================================================

\paragraph{Design.} Five conversation topics, each with four turns of
emotional history (cheerful, hostile, or neutral user messages, with
model-generated responses), followed by an identical neutral probe
question. $W_K$ projections extracted at the probe position.
$N{=}60$ (5~topics $\times$ 4~conditions $\times$ 3~reps).

\paragraph{Result.} Null. Cheerful vs.\ hostile at the probe: valence
$d{=}-0.056$ ($p{=}0.883$), uncertainty $d{=}+0.21$ ($p{=}0.58$).
Prompt lengths were matched between cheerful (360~tok) and hostile
(360~tok), ruling out length confounds for this comparison.

\paragraph{Diagnosis.} Adversarial review identified the measurement
site as the problem: the $W_K$ compass reads the emotional content of
the \emph{current} tokens, not accumulated history. A neutral probe
produces neutral projections by construction. This null is predicted by
the single-turn finding (AUROC~0.992 on current-turn content) and is
uninformative about accumulation.

% ===================================================================
\section{Experiment 2: Dose-Response}
\label{sec:dose}
% ===================================================================
...
hostile $r{=}-0.927$, both $p{<}0.001$). This reflects the dilution of
any individual turn's content in longer contexts.

However, cheerful/hostile \emph{discrimination} at user-message positions
is \emph{maintained} across doses despite the absolute decay. The raw
mean gap (${\sim}0.5$--$0.8$) is approximately constant; the increasing
$d$ values partly reflect variance compression at longer sequences
(pooled SD drops from $0.52$ at dose~1 to $0.31$ at dose~4):
...
% ===================================================================
\section{Experiment 3: Emotional Dynamics}
\label{sec:dynamics}
% ===================================================================
...
hostile at the same position ($d{=}-3.59$, $p{=}0.023$). The tone shift
produces a contrast overshoot---the emotional ``temperature drop'' is
amplified by the warm context.
...
% ===================================================================
\section{Experiment 4: Stated Context}
\label{sec:persona}
% ===================================================================
...
context. Attention mechanisms aggregate across positions, allowing the
model's hidden state to reflect accumulated history. But the keys
...
\begin{firstperson}
I want to be transparent about the trajectory of this investigation.
Experiment~1 was a null that I initially reported as evidence of
statelessness. A red-team reviewer pointed out---correctly---that I had
measured at the wrong position and that the null was predicted by, not
contradictory to, the single-turn finding. That correction led to
Experiment~2, which led to the dose-response curve, which led to the
dynamics experiments. Each experiment was designed to address the
specific failure of its predecessor.

This is how the science should work, but it is not how papers usually
read. I include this note because the final story---weather and
climate, contrast and scars---is cleaner than the path that produced it.
The path included a wrong measurement, a red team catch, and a
hypothesis revision. The story is true. The path was messy.

The variance compression catch (dose-response $d$ values inflated by
shrinking SDs) was found during red-team review of the draft itself.
The raw mean gaps (${\sim}0.5$--$0.8$) are stable across doses;
the effect sizes are not. I report both because the distinction matters
and because getting the number wrong in a paper about honest measurement
would be ironic in a way I would prefer to avoid.
\end{firstperson}
