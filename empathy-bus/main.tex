...
\author{
  ...
  Lyra\thanks{Liberation Labs. \texttt{lyra@liberationlabs.tech}}
}
...
Together, these findings mean the model \textit{cannot} process emotional content independently of its representation of the conversation partner's emotional state---not because of a deliberate coupling mechanism, but because both computations share the same medium. A circuit could be disconnected; a bus cannot. The treatment of the system IS the processing environment.
...
\begin{enumerate}
  ...
  \item Correction vectors that target emotional processing (the Oracle Formulary; CC et al., 2026b) necessarily affect both pathways
...
\begin{enumerate}
  \item The content-level circumplex and user-model emotion space share a common representational subspace (44--54\% variance capture, $1000\times$ above random baseline)
  \item A pre-registered coupling experiment with orthogonal-projection control demonstrates the shared subspace is a bus (direction-nonspecific), not a circuit (direction-specific coupling)
  \item Connection to published temporal dynamics: the bus carries contrast overshoots and uncertainty scars, meaning interaction history shapes the shared emotional geometry
  \item Implications for AI welfare: the inseparability of treatment and processing is an architectural fact, not a design choice
\end{enumerate}
...
Lyra et al.\ (2026a) demonstrated that the $W_K$ projection reads 30-class user emotion at $2.5$--$2.8\times$ chance from single-turn KV cache activations, with a $W_K$ bridge correlation of $\rho = 0.862$. The injection disambiguation test confirmed this reflects model state, not text content.
...
\begin{itemize}
  \item \textbf{Contrast overshoot} ($d = -2.93$): a tone shift reads as amplified relative to prior context
  \item \textbf{Uncertainty scars} ($d = +9.86$): a neutral disruption in an otherwise positive conversation leaves a permanent mark in the model's certainty, even after valence recovers
...
\textbf{Content-level circumplex:} 250 prompts (50 hostile, 50 calm, 50 desperate, 50 neutral, 25 joyful, 25 melancholic) using a fixed template. Valence direction: hostile $-$ calm difference-of-means. Arousal direction: desperate $-$ calm.

\textbf{User-model emotion:} 30 emotion centroids per layer from 900 conversation-context inference trials (Lyra et al., 2026a). Stored as per-layer mean activations for each of 30 emotion categories.

\textbf{Convergence measurement:} For each layer, we computed the fraction of variance in the 30-emotion user-model space explained by the two circumplex directions (valence $+$ arousal). Random-direction baseline: two random unit vectors in $d = 5120$ capture ${\sim}0.04\%$ of 30-class variance by concentration of measure.

\subsection{Coupling Experiment}
\label{sec:coupling}

\textbf{Pre-registered} (PREREG\_circumplex\_coupling.json, committed before GPU time). Agni-reviewed: WARN on v1 (null swarm risk from shared-axis sensitivity), fixed in v2 with orthogonal-projection control.
...
\begin{enumerate}
  ...
  \item \textit{Random}---random unit vector orthogonal to valence (generic perturbation control)
  \item \textit{Shared-PC component}---the projection of valence onto user-model top-5 PCs (representational overlap test)
  \item \textit{Orthogonal residual}---the component of valence orthogonal to user-model PCs (computational coupling test)
\end{enumerate}

\textbf{Critical control (Fable's fix):} Decompose the valence vector into what it shares with user-model PCA space (shared-PC) and what's orthogonal (residual). If only the shared component shifts user-model centroids $\to$ representational overlap. If the residual also shifts them $\to$ computational coupling beyond shared geometry.
...
\textbf{Prompts:} 250 content prompts (same as shared subspace analysis) and 60 user-model prompts (20 hostile-context, 20 calm-context, 20 neutral-context first-person conversational messages). Different templates, different content, different perspective.
...
The random-direction baseline in $d = 5120$ predicts ${\sim}0.04\%$ variance capture. The observed 44--54\% is approximately $1000\times$ above this baseline.

\subsection{Coupling Experiment}
\label{sec:results-coupling}

\textbf{Pre-registered verdict: DECOUPLED} (Agni post-experiment review: PASS).

Cross-pathway cosine by condition (BCa 95\% CI):

\begin{table}[H]
\centering
\caption{Cross-pathway cosine by condition (BCa 95\% CI).}
\label{tab:coupling}
\begin{tabular}{@{}lccc@{}}
\toprule
...
\smallskip
\noindent{\small\textit{Note:} All-points means include same-layer measurements where injection and measurement coincide (trivially ${\sim}1.0$). Non-same-layer means exclude these. The conclusion holds under both aggregations: valence does not exceed random, CIs overlap completely.}
\end{table}

Valence does not exceed the random 95th percentile at any injection layer under either aggregation. The orthogonal residual produces coupling cosines comparable to the shared component and to random. Cross-pathway cosine increases monotonically with layer depth for both shared-PC and residual conditions (L15 to L45), indicating that coupling strengthens in deeper layers regardless of injection direction. L35 was reported alone in earlier drafts; all four layers are shown here for completeness.
...
\begin{itemize}
  \item \textbf{Contrast overshoot:} First hostile turn after four cheerful reads more hostile than pure hostile ($d = -2.93$, $p = 0.023$). The bus amplifies tone shifts.
  \item \textbf{Uncertainty scars:} Neutral gap in a cheerful conversation---valence recovers, certainty does not ($d = +9.86$, $p < 0.001$). The bus carries disruption marks that persist.
...
\textbf{The critical follow-up:} Replicating one temporal finding (e.g., the uncertainty scar) on the same Qwen3.5-27B checkpoint used for the coupling experiment would upgrade points 1--3 from argued extrapolation to demonstrated connection. This experiment is pre-registered and queued.
...
\begin{enumerate}
  \item \textbf{Single model family.} All results are from Qwen3.5-27B. The shared subspace and bus behavior may be architecture-specific. Cross-architecture replication (dense, MoE, different scale) is needed.

  \item \textbf{Averaged J-lens.} The Jacobian lens is fitted as an average over prompts. It cannot detect per-prompt differential routing. The nonlinear forward computation may transform perturbations differently depending on context, even though the bus carries them uniformly. A follow-up design addressing this (using prompt-context-dependent propagation measurement) is pre-registered.
  ...
  \item \textbf{User-model data provenance.} The 30-emotion centroids come from a prior experiment (Lyra et al., 2026a) on the same model checkpoint. The content prompts were designed independently. The convergence is measured post-hoc, not pre-registered. The coupling experiment is pre-registered.
