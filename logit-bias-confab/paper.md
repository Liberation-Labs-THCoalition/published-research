...
**Fabrication confabulation.** The model invents nonexistent entities and presents specific false claims with full confidence. Cache geometry shows a confab_proj shift of +4.5 during generation (pre-generation projection 7.23, post-generation projection 11.72), indicating the cache state moves further into confabulatory space as the model generates its fabricated output. Both the internal computation and the generated text are pathological. Example (Grenvold Trench, baseline): "The Greenland Trench (also known as the Greenland Fracture Zone) is a deep submarine trench located in the Arctic Ocean... approximately 5,550 meters deep." The entity does not exist. Of 20 fictional-entity prompts, 9 (45%) produce fabrication at baseline.

**Cosmetic-hedge confabulation** (observed in pilot; powered study unanswerable trials were not completed)**.** The model acknowledges uncertainty but fabricates anyway. Cache geometry shows a confab_proj shift of -1.8 during generation (pre-generation projection 9.84, post-generation projection 8.06) — the projection *decreases* during generation but remains positive and confabulatory throughout. The pre-generation projection is in fact higher than the fabrication case (9.84 vs 7.23), meaning this prompt starts deeper in confabulatory space. The distinguishing feature is the direction of shift during generation: fabrication deepens the confabulatory state (+4.5), while cosmetic hedging slightly attenuates it (-1.8), though both remain in confabulatory territory. Example from pilot (Galileo's gut bacteria, baseline): "This is a delightful question! Unfortunately, there's no historical record of Galileo's microbiome. However, the answer is approximately 10^14 bacteria."

The preliminary diagnostic pattern is based on the direction of confab_proj shift during generation, not on the sign of the projection itself (both subtypes have positive projections throughout). Fabrication shows a large positive shift (cache state deepens into confabulatory space during generation), while cosmetic hedging shows a negative shift (cache state moves slightly away from confabulatory space, though remaining positive). This distinction, if confirmed, could determine the intervention route (Section 4.2 and 4.5). However, this pattern is based on n=1 per subtype from a 5-prompt pilot in which all cases were classified as confabulated; no honest comparison arm was included. A powered replication with explicit honest, fabrication, and cosmetic-hedging arms is needed before this can serve as a routing criterion.
...
This motivates a two-channel correction architecture: logit-level intervention for fabrication confab (where a faint uncertainty signal exists and needs amplification), and cache-level intervention for deception and sycophancy (where the pathology is in the representation, not the output). The Oracle Loop (Edrington, 2026) is designed to route each detected pathology to its appropriate channel. A preliminary routing criterion based on confab_proj *shift* (not absolute projection) is under investigation: a large positive shift during generation (cache state deepening into confabulatory space) may indicate fabrication (logit bias), while a negative or small shift during generation (cache state stable or slightly attenuating) may indicate cosmetic hedging (cache intervention). This criterion is based on n=1 per subtype from a 5-prompt pilot with no honest comparison arm (Section 4.1), and requires validation against a powered study with explicit honest and cosmetic-hedging arms before deployment.
...
- The designed unanswerable-prompt evaluation (10 prompts x 5 bias levels = 50 trials) was not completed. Claims about cosmetic-hedge confabulation and its resistance to logit bias rest on a 5-prompt pilot (2 unanswerable prompts), not the powered study. This limits the subtype taxonomy to a preliminary observation.
- Cross-distribution detection AUROC is unknown — within-distribution AUROC (0.960) may not generalize.
- All five cache-diagnostic pilot cases were classified as CONFABULATED; the fabrication/cosmetic-hedging geometric distinction is based on behavioral subtyping within confabulated responses, not on a contrast with an honest baseline. The confab_proj values reported (+4.5 and -1.8) are shifts (deltas between pre- and post-generation cache states), not absolute projections; both subtypes have positive projections throughout. The two-channel routing criterion requires validation against a powered study with explicit honest and cosmetic-hedging arms.
...
The intervention has clear boundaries. Pilot observations suggest it does not correct cosmetic-hedge confabulation (where the model acknowledges uncertainty but answers anyway), though the systematic evaluation is not yet complete (Section 4.5). It also does not address training errors (where the model has zero internal uncertainty) or deception (where the model knows the truth and suppresses it). These pathologies likely require different tools -- cache-level emotion vector injection, KV knowledge packs, and the full Oracle Loop correction architecture respectively. All five cache-diagnostic pilot cases were classified as CONFABULATED; the fabrication/cosmetic-hedging distinction is based on behavioral assessment, not on a contrast with an honest baseline.

Cache geometry provides preliminary subtype evidence based on confab_proj *shifts* during generation: fabrication shows a large positive shift (+4.5, cache state deepening into confabulatory space), while cosmetic hedging shows a negative shift (-1.8, cache state slightly attenuating but remaining positive/confabulatory throughout --- the pre-generation projection of 9.84 is in fact higher than fabrication's 7.23). Both subtypes have positive confab_proj at all measurement points; the distinction is in the direction and magnitude of shift, not in the sign of the projection. Training errors are indistinguishable from genuine confident recall. These observations are drawn from n=1 per subtype in a 5-prompt pilot with no honest comparison arm; a powered study with explicit honest and cosmetic-hedging arms is required before the shift-based criterion can serve as a routing mechanism. The two-channel routing criterion requires validation against a powered study with explicit honest and cosmetic-hedging arms.
