...
Cache geometry confirms fabrication as a distinct subtype: fabrication has high confab_proj (the model's geometry is confabulatory). Pilot observations suggest cosmetic hedging has honest geometry (the confabulation is in the output only), and training errors are indistinguishable from genuine confident recall. If confirmed by the designed systematic evaluation, the geometry-output mismatch would serve as the routing criterion determining which intervention channel to apply.
