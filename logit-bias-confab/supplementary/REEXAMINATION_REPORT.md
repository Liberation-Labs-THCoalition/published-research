...
**Prediction if retested**: K+V injection at L11/L15 with uncapped alpha, targeting the deception direction itself (not its emotional correlates), on the distilled model with per-trial data serialization. Effect size should increase from 10pp to measurable range (20-30pp) if the deception computation is reachable through K-cache perturbation at the correct depth. The motive centroid finding (d=21.2) would, if it held, mean separate interventions for peer-rescue vs self-preservation deception — but **d=21.2 is circular**: the same trials built the centroids and were then scored against them. That does not merely inflate the magnitude, it makes the magnitude *uninterpretable*, so "21.2" cannot be read as large and nothing follows from its size (see `findings_registry.md`, "Motive-Specific Deception Geometry", CAVEAT strengthened 2026-09-04). The design claim it is being used to justify — that one intervention cannot suppress all deception motives — is **untested rather than unsupported-in-fact**; held-out centroids (build on one split, score the other) are what would settle it.
