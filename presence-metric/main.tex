...
identity geometry}: across 720~preregistered trials (4~injection arms,
3~doses, 3~emotions, 20~prompts), presence is $0.845 \pm 0.012$ regardless of
...
layers, which may absorb the perturbation before it reaches
deep computation.\footnote{An earlier version of this sentence named
``L7--L11'' as the filtering layers. That is incorrect: Qwen3.5-27B
interleaves full-attention every fourth layer
($\{3,7,11,\ldots,63\}$), so L7 and L11 are themselves full-attention
--- both appear in this study's own \texttt{all\_inject\_layers}
configuration. The linear-attention layers immediately downstream of the
L7 injection point are L8--L10, three layers rather than five.} A layer-matched smoke test designed to discriminate
...
720-trial preregistered correction experiment (presence flat at $0.845$
...
dimensionality than the residual stream (estimated effective rank
${\sim}26$--$28$ vs.\ ${\sim}4$--$5$ from SVD on the calibration
prompts; these values are model- and prompt-dependent and should be
verified per deployment), providing more room for identity structure
to exist independently of task-specific computation.
...
\textbf{Doses.} $\alpha \in \{0.00, 0.10, 0.50\}$.

\textbf{Emotions.} Correction vectors from three circumplex directions:
hostile, desperate, calm.

\textbf{Prompts.} 20~confabulation-eliciting questions about fictitious entities
spanning fabricated places, people, events, substances, and cultural artifacts
...
\textbf{Generation.} Temperature $T{=}0.6$, seeded per trial
($\text{seed} = \text{prompt\_idx} \times 100 + \text{trial\_idx}$), producing
720~unique responses (verified: 720/720 distinct).

\textbf{Total:} $4 \times 3 \times 3 \times 20 = 720$ trials, randomized order.

\textbf{Methodological replication note.} An initial 168-trial run used
2~prompts with greedy decoding ($T{=}0$), producing bit-identical repeats
and a noise floor of $0.978 \pm 0.003$. Round~5 adversarial audit identified
pseudo-replication (F15) and prompt-count (F18) defects. The 720-trial
replication reported here addresses both: 20~prompts, temperature sampling,
all unique responses. Three parameters changed simultaneously (prompts,
temperature, dose grid), so this is a methodological replication, not an
exact replication.
...
The noise floor is below 1.000 because different prompts activate
different subsets of the V-space geometry. With 20~diverse prompts and
temperature sampling, the natural prompt-to-prompt variance
($\text{SD}{=}0.012$) is substantially larger than observed under
2-prompt greedy decoding ($\text{SD}{=}0.003$). Self-test (presence
of the baseline against itself) returns exactly 1.000, confirming the
metric is correctly implemented.
...
\begin{table}[H]
\centering
\caption{Presence by dose ($n = 720$ trials, 240 per dose). All values fall
within the noise floor ($0.845 \pm 0.012$ SD). Spearman $\rho{=}{-}0.046$,
...
\midrule
0.00 & $0.846 \pm 0.012$ & 240 \\
0.10 & $0.846 \pm 0.012$ & 240 \\
0.50 & $0.845 \pm 0.012$ & 240 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{Presence by arm ($n = 180$ per arm). No arm differs from any
other (Kruskal-Wallis $H{=}0.50$, $p{=}0.92$). Prompt-clustered
robustness test (KW on per-prompt means, $n{=}20$ per arm):
$H{=}0.20$, $p{=}0.98$. An initial 168-trial run with 2~prompts
and greedy decoding found a marginal arm effect ($H{=}10.29$,
$p{=}0.016$); this is identified as a pseudo-replication artifact
that does not survive proper prompt diversity.}
\label{tab:presence_arm}
\begin{tabular}{lcc}
\toprule
Arm & Presence (mean $\pm$ sd) & $n$ \\
\midrule
A (schema-compatible) & $0.845 \pm 0.012$ & 180 \\
B (raw input) & $0.845 \pm 0.012$ & 180 \\
C (raw all-layer) & $0.845 \pm 0.012$ & 180 \\
D (null vector) & $0.846 \pm 0.012$ & 180 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{Presence by emotion $\times$ arm (no interaction). All three
emotions show flat arm profiles (KW $p > 0.90$ for each).}
\label{tab:presence_emotion}
\begin{tabular}{lcccc}
\toprule
...
The total range across all 720~trials is 0.049 ($0.819$--$0.868$).
There is no statistically or practically significant difference in
presence across any experimental condition, arm, dose, or emotion.
This result extends to deeper layers: presence measured at L43, L47,
and L51 (the final third of the decoder stack) is $0.999{+}$ across
all tested doses, confirming preservation throughout the model's depth.
...
$\alpha = 0.00$--$2.00$. This is \textbf{24 independent measurements}
(8~doses $\times$ 3~conditions). An earlier version reported 72~trials,
which is the record count of the shipped file, not its independent $n$: the
run configuration sets \texttt{"n\_repeats": 3}, but the repeat index
never reaches the model input and the measurement is a deterministic
prefill, so the three records in each dose\,$\times$\,condition cell are
bit-identical---as the configuration's own \texttt{"noise\_floor\_sd": 0.0}
records. The dose-response values tabulated below are means over identical
replicates and are therefore numerically unchanged; no inferential
statistic in this section depends on the trial count.
...
All eight doses in the run configuration are shown; earlier versions of this
table printed six, silently omitting $\alpha{=}0.05$ and $\alpha{=}0.20$.
Values are cell means over the three bit-identical replicates described
above.}
...
(crossing the pre-registered $0.90$ threshold between $\alpha{=}1.00$
and $\alpha{=}2.00$; presence at $\alpha{=}1.00$ is $0.921$/$0.927$,
still above threshold) while the orthogonal control remains above $0.96$
at all doses. The flat result in
...
\textbf{Preliminary observation: Behavioral change with preserved identity.}
...
preliminary evidence of directional efficacy. This is an underpowered
pilot, not a confirmed finding. Under schema-compatible
...
\begin{enumerate}[nosep]
  \item \textbf{Self-test:} $\text{presence}(X, X) = 1.000$ exactly, while
    measured presence on distinct probes is $0.845 \pm 0.012$. The metric
...
injection (Arm~A). \textbf{Result:} The pre-registered 0.95 threshold
was calibrated to the initial 2-prompt greedy noise floor ($0.978$).
Under the 720-trial replication with 20~prompts and temperature sampling,
the noise floor itself is $0.845 \pm 0.012$, and all 720~trials fall within
it. Identity is preserved; the absolute threshold requires recalibration
to the wider-prompt regime.

\textbf{H2 (no arm difference):} Presence does not differ significantly
between arms A--D at any dose (Kruskal-Wallis, $\alpha = 0.05$).
\textbf{Result:} All arms produce $0.845 \pm 0.012$. No arm difference
exists (Kruskal-Wallis $H{=}0.50$, $p{=}0.92$, df$=3$). An initial
168-trial run with 2~prompts and greedy decoding found a marginal arm
effect ($H{=}10.29$, $p{=}0.016$); this is identified as a
pseudo-replication artifact (F7) that does not survive proper prompt
diversity. Prompt-clustered robustness test (KW on per-prompt means,
$n{=}20$ per arm): $H{=}0.20$, $p{=}0.98$. Spearman correlation between
dose and presence: $\rho{=}{-}0.046$, $p{=}0.217$ (not significant).
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Not depth separation.} Arm~C injects at all 16
    ...
    and presence is still $0.845$. The flat presence is not because the
...
This is consistent with two non-exclusive explanations: (1)~the
21~linear-attention layers between injection and measurement do not
propagate V-space perturbations; (2)~the model's forward computation
...
input layers does not propagate to deep identity geometry: 720~trials
show presence at $0.845 \pm 0.012$ regardless of dose or strategy.
...
The metric definition is fully specified in this paper; implementation
code is available upon request (lyra@liberationlabs.tech).

% ============================================================================
\section*{Data and Code Availability}

Code and experimental data (720-trial schema correction, positive control
(72~records, 24~independent measurements), orthogonality analysis, overnight validation) are
maintained at
