...
% ============================================================
\begin{abstract}
Language models build internal representations of their users' emotional states during inference, but where these representations reside and how they are constructed remains unknown. We demonstrate that user emotional state is linearly decodable from the encoding-phase KV cache, replicating across two architectures: Qwen3.5-27B-Claude-Distilled (hybrid attention, 900 trials) and Mistral-7B-Instruct-v0.3 (standard attention, 900 trials). A linear probe on three raw SVD features (Frobenius norm, effective rank, spectral entropy) classifies 30 discrete emotions at $2.8\times$ chance on Qwen and $2.5\times$ chance on Mistral ($p < 0.0001$ on both, 10{,}000-permutation test with FWL token-count correction and GroupKFold cross-validation by topic). The encoding-phase signal peaks at the shallowest available attention layers (layer~3 on Qwen, layer~4 on Mistral), while the generation-phase signal peaks at mid-network depth (layer~23 on Qwen, layer~11 on Mistral). The two profiles are uncorrelated ($\rho = 0.433$, $p = 0.094$ on Qwen; $\rho = 0.002$, $p = 0.990$ on Mistral), indicating architecturally separable representations. This depth ordering---encoding before generation---is consistent with rapid, automatic categorization of the user's emotional state at the first processing opportunity, followed by deeper response-phase processing, paralleling Barrett's (2006) finding that categorical emotion perception precedes dimensional analysis in human cognition. We identify a candidate pathway: emotion vectors extracted from the residual stream, when projected through the key projection matrix $W_K$, produce principal components that separate positive from negative valence, with the bridge signal building from mid-network to deep layers ($|\rho| \geq 0.83$ at layers~27--55 on Qwen, $\rho = 0.932$ at layer~17 on Mistral). However, the geometry encodes discrete emotion identity rather than a smooth valence continuum: GroupKFold cross-validation that holds out entire emotions yields $R^2 < 0$ on both architectures, indicating within-emotion regularity rather than cross-emotion valence generalization. We use a Claude-distilled model because Anthropic's published emotion vector characterization \citep{anthropic2026emotions} provides ground-truth emotion geometry; we additionally find that functional emotion representations survive the distillation process, suggesting these are robust architectural features rather than training artifacts. These findings establish that the KV cache is not merely a computational buffer---it is a substrate for the model's evolving representation of its interlocutor.
\end{abstract}
