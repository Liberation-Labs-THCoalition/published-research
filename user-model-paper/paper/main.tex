...
\textbf{Emotion-specific geometry survives implicit prompts.} K-means
clustering ($k=30$) on delta features achieves ARI $= 1.000$ in both
...
ratio $\approx 0.5$ across all features), suggesting a more constrained
...
evidence against emotion recognition---it is evidence for
...
independently arrives at the correct emotion label. Whether this
...
\begin{enumerate}[nosep]
    ...
    item 	extbf{Emotion-labeled prompts (partially addressed).} The main experiment uses explicit emotion labels. The implicit arm (S4.8) demonstrates that emotion-specific geometry survives narrative prompts without labels, but with caveats: single prompt per emotion (no within-emotion variance), and 63% of responses generate the emotion word during output. A follow-up with diverse prompts and logit-banned generation is in progress.
    ...
    item 	extbf{Encoding depth ordering (partially addressed).} The encoding-phase signal peaks at the shallowest available layers, consistent with prompt-level pattern matching when explicit labels are present. The implicit arm (S4.8) shows emotion-specific geometry without labels, suggesting the shallow-layer signal may include genuine emotion recognition alongside word-spotting. Disentangling these contributions requires the forthcoming trajectory analysis.
