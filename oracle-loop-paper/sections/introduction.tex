...
\subsection{From Monitoring to Intervention}

Detection alone is a safety tool. Combined with activation steering,
it becomes an intervention architecture.
The Oracle Loop integrates detection and correction into a runtime cycle:
...
This paper validates Steps 1--2 with a clean detection experiment
across three architectures, and demonstrates Step 3 with proof-of-concept
steering validated on two models (distilled and base) totaling 450 trials.
A future extension incorporating geometry-aware GRPO training---teaching
the model to avoid confabulation geometry from the inside---is
architecturally designed but not yet converged, and is deferred to
a companion paper.
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Proof-of-concept activation steering} with five arms
    ...
    (\cref{sec:steering}).
  \item \textbf{Cross-model steering validation} on Qwen2.5-7B-Instruct
    (350 trials, 68 baseline confabulations), confirming hostile as a
    near-universal corrector (95.6\%, McNemar $p < 0.001$) and revealing
    therapeutic inversion across architectures (\cref{sec:steering}).
