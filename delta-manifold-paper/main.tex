...
manifolds~\citep{goodfire2026}. Our findings are structurally
consistent with this framework:

\begin{itemize}
\item The \textbf{expansion-compression cycle} is consistent with the
  model traversing different manifold regions at different layers---early
  layers map input to a broad manifold region, late layers compress
  to a task-specific output manifold.

\item \textbf{Delta features} measure how far the model moves on the
  manifold during generation. Grounded generation traverses a
  substantial distance (the cache changes); confabulation stays near
  the origin (the cache does not change because the model never
  engages the knowledge manifold).

\item The \textbf{approach-and-slide trajectory} of confabulation is
  consistent with the model briefly entering the retrieval manifold
  and failing to find stable footing---sliding off into a
  low-dimensional generative mode, analogous to the ``voids'' that
  \citet{goodfire2026} describe when linear steering pushes
  representations off-manifold.

\item The \textbf{detour ratio} ($1.24$) indicates that cognitive-state
  conditions are arranged non-linearly in feature space, consistent
  with a curved manifold rather than a linear subspace.
\end{itemize}

We emphasize that this evidence is \emph{structural}, not
\emph{direct}. We have not measured manifold curvature in KV-cache
space. The connection between SVD-based spectral features and
activation-space manifold geometry remains an open question. A
definitive test would jointly analyze manifold structure (via the
methods of \citeauthor{goodfire2026}) and spectral shape (via our SVD
pipeline) on the same prompts, testing whether manifold curvature
predicts spectral shape.
...
Trajectory analysis reveals suggestive geometric structure:
confabulation shows a distinctive approach-and-slide pattern
(decline/rise ratio 1.91), and pairwise distances hint at non-linear
arrangement (detour ratio 1.24, pending bootstrap confirmation).
These observations are consistent with curved manifold structure but
do not constitute direct measurement.
