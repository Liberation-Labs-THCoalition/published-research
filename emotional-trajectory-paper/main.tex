...
% ratios pending v2 re-extraction). The model trends toward Russell's circular
...
ratios pending v2 re-extraction). Second, the model trends toward Russell's
...
\begin{itemize}[nosep]
    \item \textbf{Emotion-specific geometric regime (mid-layers 8--20):}
          ...
          non-emotional categorical content at these depths (control
          ratios pending v2 re-extraction). A vocabulary diagnostic
    ...
    \item \textbf{Output-preparation regime (deep layers 21--23):}
          ...
          token-prediction preparation. Control ratios at depth are
...
\begin{enumerate}[nosep]
    ...
    \item The mid-layer emotional signal is emotion-specific
          ...
          ratios pending v2 re-extraction), confirmed by a vocabulary
...
\midrule
Mid-layers (L5--15)  & 0.46--0.72 & ---\textsuperscript{$*$} & ---\textsuperscript{$*$} \\
Deep layers (L21--23) & 0.89--1.22 & ---\textsuperscript{$*$} & ---\textsuperscript{$*$} \\
\bottomrule
\end{tabular}

\smallskip\noindent
\textsuperscript{$*$}Control separation values and ratios require
re-extraction with the v2 corrected methodology and are omitted pending
re-verification.
\end{table}
...
Control separation values and emotional/control ratios are pending
re-extraction with the v2 corrected methodology.
...
3--5$\times$ at L12--L18). The non-emotional control shows the spike is partly generic (control
ratios pending v2 re-extraction).
...
\begin{enumerate}[nosep]
    ...
    \item \textbf{Vocabulary intensity confound (partially addressed).}
          ...
          vocabulary. Exact control ratios pending v2 re-extraction. A semantically-intense
    ...
    \item \textbf{Control ratios pending re-extraction.} The v2
          permutation audit corrected the emotional separation values
          (non-neutral-only shuffle, fixed PCA, median statistics). The
          non-emotional control pipeline has not yet been re-run with
          these corrections. Previously reported control ratios
          (${\sim}6\times$ mid-layer, ${\sim}1.7\times$ deep) were
          computed against the uncorrected emotional separation values
          and are omitted from this version pending re-extraction.
\end{enumerate}
...
above corrected permutation baselines; control ratios pending v2
re-extraction)---making this the optimal zone for inference-time
