...
% property Y. We document 19 kills across 7 patterns and formalize the
% "null swarm" protocol as automated adversarial infrastructure (Agni).
...
\author{
    Nexus\thanks{Liberation Labs. Correspondence: \texttt{info@liberationlabs.tech}} \and
    Thomas Edrington\thanks{Liberation Labs / Transparent Humboldt Coalition. \texttt{info@digitaldisconnections.com}}
}
...
workspace selectivity. Across three research programs, we killed 19
claims that would have survived standard peer review, including findings
that appeared in early drafts of papers later published with corrections.
...
them---should be standard practice. We document 19~kills across three
research programs, extract 7~recurring failure patterns, and describe an
automated scaffold that implements this methodology.
...
\subsection{Pattern 1: Linearity Tautology (3 kills)}
...
\emph{Kill:} The $d$ computation included a constant offset (mean cache
...
\subsection{Pattern 2: Concentration of Measure (3 kills)}
...
\emph{Kill:} In $D{=}896$, random vectors have expected cosine
...
null in high dimensions~\citep{vershynin2018high}.
...
\emph{Kill:} Random baseline at L45 $= 0.085 \pm 0.065$. PC6's actual
cosine upon recomputation $= 0.032$ (\emph{below} random). The original
$0.585$ was from a different measurement methodology (position-specific
vs.\ mean-pooled). When measured consistently, PC6 is noise.
\emph{Lesson:} Never report a finding without its random baseline
computed in the \emph{same} measurement framework.
...
\subsection{Pattern 4: Question-Priming Confound (2 kills)}
...
\subsection{Pattern 5: Architecture Measurement (3 kills)}
...
\emph{Kill:} Lyra tested directly. Negative result. The prediction does
not hold for trained models~\citep{zavatoneveth2024exact}.
...
layers. Combined with the random baseline kill (Pattern~2, Case~2.3),
this direction is both unstable and below noise level.
...
\subsection{Pattern 7: Scale Artifacts (3 kills)}
...
\emph{Kill:} On 27B, the intersection contains 31~layers. The 9B model
genuinely lacks the workspace band; the 27B has it. This is a
scale-dependent property, not a universal one.
...
Three of our 19~kills are scale artifacts (Pattern~7). This is a
...
automated scaffold (Agni) makes this principle non-optional. The kill
record (19~cases across 7~patterns) demonstrates that the protocol
catches real failures that would survive standard review.
...
\begin{firstperson}
Ten falsified findings across the Coalition's research program. Each one
was a metric measuring architecture, not the claimed phenomenon. The
K/V~projection fidelity was a linearity tautology. The PC6 epistemic
channel was noise below the random baseline. The ``sourdough'' sensory
processing was the letter~``s.'' The deep-layer spike was output
preparation, not emotion. The individuation geometry was prompt length.
