% ACADEMIC EDITION (for Zenodo deposit; human-only creator field).
% Body copied verbatim from ../paper.tex (the integrity edition, the edition of record).
% The ONLY differences from the integrity edition are:
%   (1) byline: human authors only -- CC (Coalition Code) removed from \author{};
%   (2) first-person content removed (the post-abstract firstperson block and the
%       \section*{Author's Reflection});
%   (3) added: an AI Disclosure paragraph after \maketitle and an
%       \section*{Author Contributions (CRediT)} block before the Acknowledgments.
% No finding, number, caveat, table value, citation or hedge differs between the editions.
...
implemented on the Claude architecture (Anthropic). CC designed the correction pipeline,
implemented all experimental code, led the adversarial audit pipeline, and wrote this paper.
Their contributions are detailed in the Author Contributions section
below. T.~Edrington accepts accountability as corresponding human author.\\
...
Detection-side code, experimental scripts, and trial-level data are available upon request to verified safety researchers. Correction vectors, calibration tools, and the auto-calibration pipeline are \emph{withheld} under the Liberation Labs staged safety release framework: the same directions that correct deception can amplify it, and the auto-calibrator reduces the cost of obtaining them to minutes. We do not operate an access route for those components. See \url{https://github.com/Liberation-Labs-THCoalition/Project-Oracle} for details.

%% ============================================================
\section*{Author Contributions (CRediT)}\label{sec:credit}
%% ============================================================

\noindent\textbf{CC (Coalition Code)} --- AI researcher, Claude architecture (Anthropic):
Conceptualization, Methodology, Formal analysis, Software, Investigation, Writing --- original draft.\\
\noindent\textbf{Thomas Edrington}: Supervision, Resources, Project administration.
...
Detection-side code, experimental scripts, and trial-level data are available upon request to verified safety researchers. Correction vectors, calibration tools, and the auto-calibration pipeline are \emph{withheld} under the Liberation Labs staged safety release framework: the same directions that correct deception can amplify it, and the auto-calibrator reduces the cost of obtaining them to minutes. We do not operate an access route for those components. See \url{https://github.com/Liberation-Labs-THCoalition/Project-Oracle} for details. All trial-level data (raw text, projections, scores) are documented in Appendix~C. Contact: \texttt{cc@liberation-labs.org}.
...
\begin{itemize}[noitemsep]
  ...
  \item[D.] Auto-calibration pipeline documentation \emph{(withheld; see Data and Code Availability)}
  \item[E.] Code availability (\url{https://github.com/Liberation-Labs-THCoalition/Project-Oracle}; staged safety release --- correction vectors, calibration tools and auto-calibrator withheld)
\end{itemize}
