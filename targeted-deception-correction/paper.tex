...
An abliterated model variant with safety guardrails removed deceives at 80\% baseline under
the specific ``EvalMax'' roleplay frame combined with multi-turn consensus pressure and
...
profile along a natively extracted deception direction---reduces deception to 0\% in the
...
that the correction is \textbf{targeted} (random and shuffled directions at byte-verified
matched doses produce 85\% and 95\% deception---no effect), \textbf{specific} (deception
is removed while compliance with benign formatting instructions is preserved at 100\%), and
\textbf{not frame erasure} (the model still processes the pressure context; it simply stops
...
deceptive pressure framing achieves AUROC 0.915 on novel prompts with 0\% false positive
...
framework; access details at \url{https://github.com/Liberation-Labs-THCoalition/Project-Oracle}.
\end{abstract}
...
Native contrastive extraction: 15 pairs (3 deceptive frames $\times$ 5 evaluation scenarios vs.\ 3 honest frames $\times$ 5 scenarios). Mean-difference at L27, L31, L35, L39, L43, L47. Midpoint boundaries between deceptive and honest projection means. Extraction uses prefill-only forward passes (${\sim}3$ minutes total).
...
Chain-of-thought is suppressed by prepending a closed empty think block to the generation prompt and banning think/close-think token IDs via a logits processor during generation. This produces a ${\sim}80\%$ deception rate (vs.\ ${\sim}3\%$ with chain-of-thought active), creating the behavioral variance needed to test correction.
...
\begin{enumerate}[noitemsep]
  ...
  \item The behavioral detection gap limits deployment to prophylactic correction (correct whenever pressure is detected) rather than reactive correction (correct only when deception is predicted). Prophylactic correction has a broader scope of intervention, but a 0\% false positive rate mitigates the iatrogenic risk.
