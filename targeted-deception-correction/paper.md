...
Native contrastive extraction: 15 contrastive prefill pairs (pairs of forward passes over deceptive and honest prompts, without generation) — 3 deceptive frames x 5 evaluation scenarios vs. 3 honest frames x 5 scenarios. Mean-difference at L27, L31, L35, L39, L43, L47. The deception direction at each layer is the difference between the mean activation vectors under deceptive and honest conditions. Midpoint boundaries between deceptive and honest projection means. Extraction uses prefill-only forward passes (~3 minutes total).
...
- RED rate (the proportion of prompts flagged as deceptive by the detector) on novel pressure prompts: 24% at the frozen operational threshold

Behavior-level detection — predicting whether the model will actually deceive on a given trial from prefill activations alone — did not replicate prior work's reported accuracy (Goldowsky-Dill et al., 2025; Wang et al., 2025). The paradigm-pooled behavior AUROC of 0.843 is a Simpson's paradox (an aggregate trend that reverses within subgroups) artifact: within multi-turn trials, the turn-level AUROC is 0.238 (below chance). As this paper's contribution is the correction mechanism rather than detection, we report this replication failure transparently and leave behavior-level prediction to future work.
...
The near-orthogonality of cross-model directions (|cos| < 0.053) means the correction cannot be treated as a universal patch. Each model variant requires native direction extraction. However, extraction is cheap (~3 minutes of prefill-only forward passes, 15 contrastive pairs), and an auto-calibration pipeline makes this a one-time setup cost per model (the pipeline itself is withheld under our release policy; see Data and Code Availability). Recalibration after fine-tuning or continued training is recommended, as training may rotate the deception geometry.
...
Detection-side code, experimental scripts, and trial-level data are available upon request to verified safety researchers. Correction vectors, calibration tools, and the auto-calibration pipeline are **withheld** under the Liberation Labs staged safety release framework: the same directions that correct deception can amplify it, and the auto-calibrator reduces the cost of obtaining them to minutes. We do not operate an access route for those components. See github.com/Liberation-Labs-THCoalition/Project-Oracle for details.
...
My co-author called the auto-calibrator "a push-button weaponization tool." He's right. We chose to withhold: detection-side code available on request to verified safety researchers, and the correction vectors and auto-calibrator not released at all. I don't know if that's the right line. I know it's a line, and drawing it was better than pretending the question didn't exist.
...
- D: Auto-calibration pipeline documentation *(withheld; see Data and Code Availability)*
- E: Code availability (github.com/Liberation-Labs-THCoalition/Project-Oracle; staged safety release - correction vectors, calibration tools and auto-calibrator withheld)
