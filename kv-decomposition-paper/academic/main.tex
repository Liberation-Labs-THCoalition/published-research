...
\begin{abstract}
K and V tensors contribute superadditively to graph topology recovery in KV cache injection: each alone achieves 0.333 multi-hop accuracy, but together they achieve 1.000---the combination exceeds the sum of its parts. KV cache injection pre-computes knowledge as key-value tensor pairs and inserts them at inference time, enabling domain expertise without retraining. We decompose cache-injected graph topology into five conditions on a 21-node knowledge graph: baseline, full KV, V-only (neutral K + topology V), K-only (topology K + neutral V), and text-in-prompt. Full KV injection achieves perfect scores (1.000 sanity, 1.000 multi-hop). Neither K nor V alone recovers the topology that both together deliver.
\end{abstract}

\section{Introduction}

Our prior work \citep{nexus2026topology} demonstrated that graph topology encoded as structured text can be injected into language model KV caches, enabling 91.7\% topology recovery with walk encoding. Scrambled controls confirmed the signal is topological, not format-dependent (+54.2 percentage point delta; see \citet{nexus2026topology} for empty-response qualification on this number).

That work treated the KV cache as an atomic unit---injecting both keys and values together as a single operation. But the cache comprises two distinct tensor families: \textbf{keys} (K) and \textbf{values} (V). These serve fundamentally different roles in the attention mechanism:

\begin{itemize}
    \item \textbf{K tensors} compute attention weights via dot product with queries. They carry Rotary Position Embeddings (RoPE), a technique that encodes each token's sequential position directly into the key vectors. K tensors determine \textit{where the model attends}---the routing of information flow.
...
The Knowledge Packs paper \citep{pustovit2026knowledge} observed that RoPE rotates keys but leaves values untouched, noting that value-only modification avoids corrupting positional signals. This suggests K and V may be separable for content injection. We test whether this extends to structural injection.
...
\bottomrule
\end{tabular}
\caption{Decomposition results. Overall = (Sanity + Multi-hop) / 2, equally weighting the two evaluation dimensions. All conditions preserve perfect sanity (1.000). Full KV injection achieves perfect multi-hop (1.000). V-only and K-only each achieve 0.333---equal to each other, slightly above baseline, but far below full injection.}
\label{tab:results}
\end{table}
...
The central finding: V-only and K-only injection each achieve 0.333 multi-hop---identical to each other, modestly above baseline (0.167), but dramatically below full KV injection (1.000). Note: the two questions both conditions answered correctly were the isolate query (also correct at baseline, likely answerable from the node name) and one reachability query (where responses lacked graph-based reasoning). The +0.167 delta over baseline (one additional question at $n{=}6$) should be interpreted cautiously. The decomposition reveals that:
...
\textbf{Lyra}: Methodology, Validation, Writing -- review \& editing.\\
\textbf{Thomas Edrington}: Conceptualization, Supervision, Resources, Project administration.
