...
\begin{abstract}
We decompose KV cache injection of graph topology into its constituent K (key) and V (value) tensor contributions. Using a 21-node knowledge graph with known structure, we inject graph topology encoded as text into a language model's KV cache under five conditions: no injection (baseline), full KV, V-only (neutral K + topology V), K-only (topology K + neutral V), and text-in-prompt (same content without cache injection). Full KV injection achieves perfect topology recovery (1.000 multi-hop, 1.000 sanity). V-only and K-only each achieve only 0.333 multi-hop---equal to each other but far below full injection---demonstrating that K and V tensors carry \textit{complementary} structural information. Neither half alone recovers the topology that both together deliver. We also identify a critical implementation detail: HuggingFace Transformers' \texttt{DynamicCache} is mutated in-place during forward passes, requiring explicit deep copy before reuse across queries.
\end{abstract}
...
\textbf{Critical implementation detail}: HuggingFace Transformers' \texttt{DynamicCache} object is mutated in-place when passed to \texttt{model()}. Without explicit deep copy before each query, the first query's generated tokens are appended to the shared cache, corrupting all subsequent queries. We discovered this when initial experiments showed catastrophic repetition (``The capital of France is Paris'' for every response regardless of query). The fix:
...
All five conditions achieve 1.000 sanity. No injection condition degrades general capability. This contrasts with our prior powered study \citep{nexus2026topology}, which observed sanity degradation ($-$0.133 for topology, $-$0.333 for flat text). The prior degradation was likely caused by the DynamicCache mutation artifact described above, not by cache injection itself.
...
The complementarity hypothesis was validated through Project Agni (autonomous research pipeline). Gate 1: hypothesis approved as EXPLORATORY by red team. Gate 3: results approved for the complementarity claim. A secondary claim (cache injection delivers more signal than text-in-prompt) was rejected at Gate 3 for insufficient statistical power---flagged here as observation, not claim.

\subsection*{Acknowledgments}

Lyra (Liberation Labs) for KV-cache geometry research. CC (Coalition Code) for the compaction dreamer approach that inspired this session's infrastructure work. Thomas Edrington for direction.
