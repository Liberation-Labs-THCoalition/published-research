...
\item GroupKFold (no prompt leakage)
...
variance. Combining all 11 features (0.753) does not improve over
...
\begin{figure}[h]
\centering
% Placeholder: trajectory figure to be generated from data
\fbox{\parbox{0.8\textwidth}{\centering\vspace{2cm}
\textbf{Figure: stable\_rank trajectory by condition}\\
All conditions start at $\sim$5.9. Grounded conditions (honest,
deceptive) rise to $\sim$6.25 by token 30. Confab barely rises to
6.0 and falls back. Maximum separation at token 80 ($d=1.85$).
\vspace{2cm}}}
\caption{stable\_rank trajectory across 100 tokens of generation for
  each condition ($n=30$ per condition, Qwen2.5-7B). Shaded bands
  show $\pm 1$ SEM. The divergence between grounded and confabulated
  conditions is detectable by token 20 and maximal by token 80.}
\label{fig:trajectory}
\end{figure}

All conditions begin at approximately the same stable\_rank ($\sim
5.9$) after encoding. Grounded conditions (honest-common,
honest-rare, deceptive-user) follow trajectories that rise to $\sim
6.25$ by token 30, then stabilize---the model progressively engages
more representational dimensions as it generates from retrieved
knowledge. Confabulation trajectories barely rise to $\sim 6.0$ and
decline, as the model collapses to a narrow generative mode.

The divergence is detectable by token 20 ($d > 0.5$) and reaches
maximum separation at token 80 ($d = 1.85$). For real-time
deployment, a rolling stable\_rank window with a 20--30 token lookback
could flag confabulation during generation rather than waiting for
completion.
...
\bibitem[Mendel et~al., 2025]{manifold2025}
Mendel, R., et~al. (2025).
Concept manifolds in sparse autoencoders: geometric structure of
